...
namespace CyberTechShop.Controllers
{
    public class CategoryManageController : Controller
    {
        ...
        /// <summary>
        /// Lấy toàn bộ cây danh mục với thống kê
        /// </summary>
        [HttpGet]
                ...
                var categories = await _context.Categories
                    .Include(c => c.Subcategories)
                        .ThenInclude(sc => sc.SubSubcategories)
                            .ThenInclude(ssc => ssc.Products.Where(p => p.Status == "Active"))
                    .Select(c => new
                    {
                        id = c.CategoryID,
                        name = c.Name,
                        ...
                        productCount = c.Subcategories
                            ...
                            .SelectMany(ssc => ssc.Products.Where(p => p.Status == "Active"))
                            .Count(),
                        subcategories = c.Subcategories.Select(sc => new
                        {
                            ...
                            productCount = sc.SubSubcategories
                                .SelectMany(ssc => ssc.Products.Where(p => p.Status == "Active"))
                                .Count(),
                            subSubcategories = sc.SubSubcategories.Select(ssc => new
                            {
                                ...
                                productCount = ssc.Products.Count(p => p.Status == "Active")
                            }).ToList()
                        }).ToList()
                    })
                    .ToListAsync();
        ...
        public async Task<IActionResult> DeleteCategory(int id, string type)
        {
            ...
            try
            {
                ...
                var message = $"Đã xóa thành công: {deletedProducts} sản phẩm";
                if (deletedSubSubCategories > 0) message += $", {deletedSubSubCategories} danh mục con";
                if (deletedSubCategories > 0) message += $", {deletedSubCategories} danh mục phụ";
